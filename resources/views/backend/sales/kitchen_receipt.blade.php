...
<table width="80%" cellpadding="10" class="tableS" cellspacing="10" style="font-family: Times New Roman; font-size: 11.5px !important;margin-left:20px" >
 <tr>
    <td colspan="5" style="text-align:center" class="noborder"><img src="{{url('uploads/logo.jpg')}}" width="220" alt="PRA"></td>
...
<table width="90%" cellpadding="10" class="tableS" cellspacing="10" style="font-family: Times New Roman; font-size: 11.5px !important;margin-left:20px;">
 ...
 <tr>
    <td colspan="5" class="noborder"><strong>@lang('slip.date')</strong> <?php echo date('d/m/Y H:i:s'); ?></td>
     
 </tr>
 <tr>
    <td colspan="2"  width="40%" class="noborder"><strong>@lang('Sala: ')</strong> <?php echo $table->room_id; ?></td>
    <td colspan="3" style="float:right;" width="60%" class="noborder"><strong>@lang('Tavolo:')</strong> <?php echo $table->table_id; ?></td>
 ...
 <tr>
    ...
    <td width="100"><strong>@lang('slip.menu')</strong></td>
    <td id="kitchenph" width="50"><strong>@lang('slip.unit_price')</strong></td>
	<td width="15"><strong>@lang('slip.qty')</strong></td>
    <td id="kitchentotalh" width="50"><strong>@lang('slip.total')</strong></td>
 </tr>
</thead>
    ...
    <?php $i=1; $subtotal = 0;?>
                    ...
                    <tr height colspan="5" class="tableStyle">
                      ...
                      <tr>
                            ...
                            @if($c["deleted"] == 1) 
                            <td colspan="1" width="100"><del>{{$c["size"]}}</del><br><small>{{$c["note"]}}</small></td>
                            @else
                            <td  colspan="1" width="100"><strong>{{$c["size"]}}</strong><br><small>{{$c["note"]}}</small></td>
                            @endif
                            
                            <td class="kitchen" width="50"><strong><?php echo $currency; ?>{{$c["price"]}}</strong></td>
							<td width="15"><strong>{{$c["quantity"]}}</strong></td>
                            <td class="kitchen" width="50"><strong><?php echo $currency; ?>{{ number_format($c["quantity"] * $c["price"],2) }}</strong></td>
                      </tr>
                       
                       <?php $subtotal = $subtotal + ($c["quantity"] * $c["price"]);  $i++; ?>
                       @endforeach
...
<table style="page-break-inside:avoid;font-family: Times New Roman; font-size: 11.5px !important;margin-left:20px" width="90%" cellpadding="5" class="tableS kitchen" cellspacing="5" id="kitchen">
     ...
     <tr>
        ...
        <td><strong></strong></td>
        <td class="grandtotalFont"><strong><?php echo $currency; ?>{{number_format($subtotal ,2)}}</strong></td>
    </tr>   
    <?php /* <tr>
    <td></td>
    <td></td>
 ...
 <tr>
  ...
  <tr>
    <td colspan="5" align="center">
    <strong> {{setting_by_key('title')}}, {{setting_by_key('address')}}, {{setting_by_key('phone')}}</strong></td>
...
<p align="center"><input style="font-size:<?php echo $fontSize; ?>" type="button" id="pr" value="Stampa" onclick="printpage()" class="btn btn-success" /> </p>
  ...
  <script type="text/javascript" src="{{url('assets/frontend/js/jquery-1.11.1.min.js')}}"></script><!-- jquery -->
    <script type="text/javascript">
        function printpage() {
            ...
            $.ajax({
                    type: 'POST',
                    url: '<?php echo url("print/kreceipt"); ?>',
                    data: form_data,
